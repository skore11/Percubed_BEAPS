...
namespace Percubed.Flex
{
    ...
    public class ShockEffectFlex : MonoBehaviour//, IStorable
    {
        ...
        void OnFlexUpdate(FlexContainer.ParticleData _particleData)
        {
            ...
            if (keyboard != null)
            {
                if (keyboard.jKey.isPressed || shock)
                {
                    for (int pId = 0; pId < m_particles.Length; pId++)
                    {
                        ...
                        m_velocities[pId].y = (Random.value - 0.5f) * MAX_SHOCK_FACTOR * m_particles[pId].w;
